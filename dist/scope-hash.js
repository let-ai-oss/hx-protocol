 ...
 */
export function hashFortressScope(scope) {
    const identities = Array.isArray(scope?.identities) ? scope.identities : [];
    const tuples = identities
        .map((x) => [String(x.userExternalId), String(x.family), String(x.sessionId)])
        .sort(cmpTuple);
    const gate = scope?.ownerGate && Array.isArray(scope.ownerGate.activeMemberExternalIds)
